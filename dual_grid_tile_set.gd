...
## Checks if a bespoke mix exists between two source IDs
## Returns Array [primary_source_id: int, atlas_offset: int] or an empty Array if no mix exists
func get_bespoke_mix(source_a: int, source_b: int) -> Array:
	...
	var key_a := Vector2i(source_a, source_b)
	if _mix_cache.has(key_a):
		return [source_a, _mix_cache[key_a]]

	var key_b := Vector2i(source_b, source_a)
